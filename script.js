      ...
      entries.forEach((entry) => {
        if (
          ...
          cards.forEach((card, index) => {
            card.style.animation = `floatingAnimation 6s ease-in-out infinite ${
              index * -2
            }s`;
          });
...
function seleccionar(event) {
  ...
  document
    .querySelectorAll(".contenedor-header nav ul li a")
    .forEach((link) => {
...
function updateActiveSection() {
  ...
  sections.forEach((section) => {
    ...
    if (scrollY > sectionTop && scrollY <= sectionTop + sectionHeight) {
      document
        .querySelector(`.contenedor-header nav ul li a[href*=${sectionId}]`)
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Funcionalidad para el botón de ver más cursos
  const verMasBtn = document.getElementById("verMasCursos");
  const cursosAdicionales = document.getElementById("cursosAdicionales");
  const textoBoton = verMasBtn.querySelector(".texto-boton");

  if (verMasBtn && cursosAdicionales) {
    verMasBtn.addEventListener("click", function () {
      const estaMostrando = cursosAdicionales.style.display === "block";

      if (estaMostrando) {
        cursosAdicionales.style.display = "none";
        cursosAdicionales.classList.remove("mostrar");
        textoBoton.textContent = "Ver más cursos";
        verMasBtn.classList.remove("activo");
      } else {
        cursosAdicionales.style.display = "block";
        // Pequeño retraso para que la animación funcione correctamente
        setTimeout(() => {
          cursosAdicionales.classList.add("mostrar");
        }, 10);
        textoBoton.textContent = "Ver menos cursos";
        verMasBtn.classList.add("activo");
      }
    });
  }
});
